...
export default async function SalesPage() {
  ...
  const [orders, photos, selections] = client ? await Promise.all([
    ...
    client.from('photos').select('id,filename,price_cents,galleries!inner(title,visibility)').eq('galleries.visibility', 'public').eq('processing_status','done').order('created_at',{ascending:false}).limit(500),
  ...
  return <main id="conteudo-principal" className="min-h-screen px-6 pb-24 pt-40"><div className="mx-auto max-w-5xl"><h1 className="font-display text-5xl">Vendas e seleções</h1><Link className="button-secondary my-6" href="/loja">Abrir loja</Link>
    ...
    {photos?.error ? <p>Preços indisponíveis. Verifique a migração do banco.</p> : photos?.data?.map(photo => <article key={photo.id} className="mb-4 border border-white/10 p-5"><h3>{photo.filename} · {photo.id.slice(0,8)}</h3><SalesControls photoId={photo.id} price={photo.price_cents} /></article>)}
  </div></main>;
}
