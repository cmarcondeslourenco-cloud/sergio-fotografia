...
export default async function GalleryManagementPage({ params }: { params: { slug: string } }) {
  ...
  const { data: photos } = await client.from('photos').select('id,filename,is_cover,is_featured,processing_status,path_preview').eq('gallery_id', gallery.id).order('sort_order');
