...
const schema = z.discriminatedUnion('action', [
  z.object({ action: z.literal('price'), photoId: z.string().uuid(), cents: z.number().int().min(1).max(10000000).nullable() }),
...
export async function POST(request: Request) {
  ...
  if (body.action === 'price') {
    const { data, error } = await admin.from('photos').update({ price_cents: body.cents }).eq('id', body.photoId).select('id').single();
    if (error || !data) return NextResponse.json({ error: 'Não foi possível salvar o preço.' }, { status: 409 });
  } else {
    const { data, error } = await admin.from('photo_orders').update({ status: body.action, ...(body.action === 'paid' ? { paid_at: new Date().toISOString(), confirmed_by: user.id, expires_at: new Date(Date.now() + 30 * 86400000).toISOString() } : {}) }).eq('id', body.orderId).eq('status', 'pending').select('id').maybeSingle();
