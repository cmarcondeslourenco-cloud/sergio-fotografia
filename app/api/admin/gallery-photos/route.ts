...
export async function DELETE(request: Request) {
  ...
  if (paths.length) {
    ...
    if (storageError) return NextResponse.json({ error: 'Não foi possível remover os arquivos privados selecionados.' }, { status: 502 });
  }
  const { error: deleteError } = await admin.from('photos').delete().eq('gallery_id', body.galleryId).in('id', photoIds);
  if (deleteError) return NextResponse.json({ error: 'Não foi possível excluir as fotos selecionadas.' }, { status: 502 });
