...
export async function PortfolioGallery({ slug, fallback, showAllPhotos = false }: { slug: string; fallback: ReactNode; showAllPhotos?: boolean }) {
  ...
  const { data: photos } = await admin.from('photos').select('id,filename,path_preview,is_featured').eq('gallery_id', gallery.id).eq('processing_status', 'done').eq('published', true).order('sort_order').limit(portfolioMaxGalleryPhotos);
